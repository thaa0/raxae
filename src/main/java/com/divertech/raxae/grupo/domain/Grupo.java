...
public class Grupo {
    ...
    public boolean jaEhMembro(Usuario usuario) {
        return this.membros.stream()
            .anyMatch(membro -> membro.getUsuario().getId().equals(usuario.getId()));
    }
    
    public boolean isAdmin(UUID idUsuario) {
        return this.administrador != null && this.administrador.getId().equals(idUsuario);
    }
