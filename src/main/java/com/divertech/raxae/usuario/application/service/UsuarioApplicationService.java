...
public class UsuarioApplicationService implements UsuarioService {
        ...
        public InfoUsuarioResponse getInfoUsuario(Usuario usuarioLogado, YearMonth mes) {
                ...
                double economiaComoParticipante = todasCobrancasPagas.stream()
                ...
                var todasCobrancasRecebidas = cobrancaRepository.buscarCobrancasPagasRecebidasPorUsuario(
                                usuarioLogado.getId());

                double economiaComoAdmin = todasCobrancasRecebidas.stream()
                                .map(Cobranca::getValor)
                                .map(BigDecimal::doubleValue)
                                .reduce(0.0, Double::sum);

                double economiaTotal = economiaComoParticipante + economiaComoAdmin;
